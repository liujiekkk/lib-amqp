...
trait ConnectionMethods {
     ...
     * @return Buffer
     */
    abstract public function getWriteBuffer(): Buffer;
    ...
    protected function authResponse(MethodConnectionStartFrame $start)
    {
        if (strpos($start->mechanisms, "AMQPLAIN") === false) {
            throw new ConnectionException("Server does not support AMQPLAIN mechanism (supported: {$start->mechanisms}).");
        }
     ...
     */
    protected function awaitConnectionStart()
    {
        ...
        $this->addAwaitCallback(function ($frame) use ($deferred) {
            if ($frame instanceof Protocol\MethodConnectionStartFrame) {
                ...
                $this->connectionCloseOk()
                    ->done(function () use ($frame, $deferred) {
                    $deferred->reject(new ChannelException($frame->replyText, $frame->replyCode));
                });
                return true;
            }
            return false;
     ...
     */
    protected function awaitConnectionTune()
    {
        $deferred = new Deferred();
        $this->addAwaitCallback(function ($frame) use ($deferred) {
            if ($frame instanceof Protocol\MethodConnectionTuneFrame) {
                ...
                $this->connectionCloseOk()
                    ->done(function () use ($frame, $deferred) {
                    $deferred->reject(new ConnectionException($frame->replyText, $frame->replyCode));
                });
                return true;
            }
            return false;
     ...
     */
    protected function awaitConnectionOpenOk()
    {
        $deferred = new Deferred();
        $this->addAwaitCallback(function ($frame) use ($deferred) {
    ...
    protected function connectionStartOk($clientProperties = [], $mechanism = 'PLAIN', $response, $locale = 'en_US')
    {
        $buffer = new Buffer();
    ...
    protected function connectionSecureOk($response)
    {
    ...
    protected function connectionTuneOk($channelMax = 0, $frameMax = 0, $heartbeat = 0)
    {
        $buffer = $this->getWriteBuffer();
        $buffer->appendUint8(1);
        $buffer->appendUint16(0);
    ...
    protected function connectionOpen($virtualHost = '/', $capabilities = '', $insist = false)
    {
        $buffer = $this->getWriteBuffer();
        $buffer->appendUint8(1);
        $buffer->appendUint16(0);
    ...
    protected function connectionClose($replyCode, $replyText = '', $closeClassId, $closeMethodId)
    {
        $buffer = $this->getWriteBuffer();
        $buffer->appendUint8(1);
        $buffer->appendUint16(0);
    ...
    public function publish($channel, $body, array $headers = [], $exchange = '', $routingKey = '', $mandatory = false, $immediate = false)
    {
        ...
        if (! $c) {
            $this->cache[$ck] = [
                $buffer->read($len0, $off0),
                $buffer->read($len1, $off1)
            ];
